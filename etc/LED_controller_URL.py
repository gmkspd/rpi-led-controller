...
from flask import Flask, jsonify     # flask 모듈을 불러움
...
yellow_pin = 15                   # 초록 LED 핀은 라즈베리파이 GPIO 15번핀으로 
...
def red_on():               # 해당 페이지의 뷰함수 정의
   ...
   return jsonify({'message': 'turning on red'})

@app.route('/yellow_on')     # IP주소:port/yellow_on 을 입력하면 나오는 페이지
def yellow_on():             # 해당 페이지의 뷰함수 정의
   GPIO.output(yellow_pin, GPIO.HIGH) # 초록 LED 핀에 HIGH 신호 인가(LED 켜짐)
   # return "yellow LED on"    
   return jsonify({'message': 'turning on yellow'})

@app.route('/blue_on')      
def blue_on():              
   GPIO.output(blue_pin, GPIO.HIGH)
   # return "blue LED on"    
   return jsonify({'message': 'turning on blue'})
